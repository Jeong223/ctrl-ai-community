...
import { ArrowLeft, Check, CheckSquare, Edit3, ExternalLink, FileText, Link2, ListChecks, MessageSquareText, Plus, Target, Trash2, Users } from "lucide-react";
...
export function ProjectDetail({ id }: { id: string }) {
  const { data, saveProject, addProjectUpdate, saveProjectUpdate, removeProjectUpdate } = useCommunity();
  ...
  function openDialog(kind: Exclude<DialogKind, null>) {
    ...
    setDialog(kind);
    setTitle(""); setUrl(""); setContent(kind === "memo" ? project!.meetingNotes : "");
  }
  ...
  async function submitDialog(event: FormEvent<HTMLFormElement>) {
    ...
    if (dialog === "update") saved = editingUpdate
      ? await saveProjectUpdate(project!.id, { ...editingUpdate, title, content })
      : await addProjectUpdate(project!.id, { id: "", title, content, createdAt: new Date().toISOString() });
    ...
    <>
      ...
      <section className="card project-detail-head">
        <div className="project-title-row">
          <div><Badge tone={tone[project.status]}>{project.status}</Badge><h1>{project.name}</h1><p>{project.description}</p></div>
      ...
      <div className="project-detail-grid">
        ...
        <aside className="dashboard-stack">
          ...
          <section className="card detail-card">
            <div className="section-title"><h2><ListChecks size={17} /> 체크리스트</h2>{isAdmin && <button className="button button-secondary button-small" onClick={() => openDialog("checklist")}><Plus size={12} /></button>}</div>
            <div className="check-list">{project.checklist.map((item) => <button type="button" disabled={!isAdmin} className={`check-item ${item.done ? "is-done" : ""}`} key={item.id} onClick={() => toggleCheck(item.id)}><span className="check-dot">{item.done && <Check size={11} />}</span>{item.label}</button>)}</div>
          </section>
          <section className="card detail-card">
            <div className="section-title"><h2><Link2 size={17} /> 공유자료</h2>{isAdmin && <button className="button button-secondary button-small" onClick={() => openDialog("resource")}><Plus size={12} /></button>}</div>
