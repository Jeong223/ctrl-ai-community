...
export default function MembersPage() {
  ...
  return (
    <>
      ...
      />
      {isAdmin && <div className="admin-guide"><span><Edit3 size={16} /></span><div><strong>회원정보 관리자 편집</strong><p>카드의 수정 버튼으로 직급, 관심 분야, AI 도구와 참여 프로젝트를 바로 관리할 수 있습니다.</p></div></div>}
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title={draft.id ? "회원정보 수정" : "새 회원 추가"} description="공개 가능한 동호회 활동 정보만 입력해 주세요.">
        <form onSubmit={submit}>
          <div className="form-grid">
            ...
            <Field label="직급"><input required value={draft.role ?? ""} onChange={(event) => setDraft({ ...draft, role: event.target.value })} placeholder="전무, 차장, 과장 등" /></Field>
