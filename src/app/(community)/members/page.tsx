...
export default function MembersPage() {
  ...
  return (
    <>
      ...
      />
      {isAdmin ? <div className="admin-guide"><span><Edit3 size={16} /></span><div><strong>회원정보 관리자 편집</strong><p>카드의 수정 버튼으로 직급, 관심 분야, AI 도구와 참여 프로젝트를 바로 관리할 수 있습니다.</p></div></div> : memberId ? <div className="admin-guide"><span><Edit3 size={16} /></span><div><strong>내 프로필 편집</strong><p>내 카드의 수정 버튼으로 관심 분야, AI 도구, 참여 프로젝트와 한 줄 소개를 관리할 수 있습니다.</p></div></div> : null}
        ...
        <article className="card card-hover member-card" key={member.id}>
          <div className="member-top">
            ...
            <div className="member-card-controls">
              ...
              {(isAdmin || memberId === member.id) && <div className="member-admin-actions"><button className="button button-secondary button-small" onClick={() => openEdit(member)} aria-label={`${member.name} 회원정보 수정`}><Edit3 size={12} /></button>{isAdmin && <button className="button button-danger button-small" onClick={() => remove(member)} aria-label={`${member.name} 회원정보 삭제`}><Trash2 size={12} /></button>}</div>}
            </div>
          </div>
          <h2>{member.name}</h2>
          {member.interest && <span className="member-role">{member.interest}</span>}
          <p className="member-bio">{member.bio ? `“${member.bio}”` : "-"}</p>
          <dl className="member-detail"><div><dt>AI 도구</dt><dd>{member.aiTools?.join(" · ") || "-"}</dd></div><div><dt>프로젝트</dt><dd>{member.projects?.join(", ") || "-"}</dd></div></dl>
        </article>
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title={draft.id ? (isAdmin ? "회원정보 수정" : "내 프로필 수정") : "새 회원 추가"} description="공개 가능한 동호회 활동 정보만 입력해 주세요.">
        <form onSubmit={submit}>
          <div className="form-grid">
            <Field label="이름"><input required disabled={!isAdmin} value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></Field>
            <Field label="프로필 이니셜" hint="카드 프로필에 표시됩니다."><input required disabled={!isAdmin} maxLength={3} value={draft.initials} onChange={(event) => setDraft({ ...draft, initials: event.target.value })} placeholder="예: MJ" /></Field>
            <Field label="직급"><input required disabled={!isAdmin} value={draft.role ?? ""} onChange={(event) => setDraft({ ...draft, role: event.target.value })} placeholder="전무, 차장, 과장 등" /></Field>
            <Field label="프로필 색상"><select disabled={!isAdmin} value={draft.color} onChange={(event) => setDraft({ ...draft, color: event.target.value })}>{memberColors.map((color) => <option value={color} key={color}>{color}</option>)}</select></Field>
            <Field label="관심 AI 분야" className="field-full"><input value={draft.interest ?? ""} onChange={(event) => setDraft({ ...draft, interest: event.target.value })} placeholder="AI 자동화, 생성형 디자인 등" /></Field>
            ...
            <Field label="한 줄 소개" className="field-full"><textarea value={draft.bio ?? ""} onChange={(event) => setDraft({ ...draft, bio: event.target.value })} /></Field>
          </div>
