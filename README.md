...
기존 데이터베이스를 업그레이드하는 경우에는 [supabase/migrations/20260820_add_comments_and_writer_tags.sql](supabase/migrations/20260820_add_comments_and_writer_tags.sql)을 SQL Editor에서 한 번 실행합니다. 이 마이그레이션은 기존 게시물과 프로젝트를 보존하면서 댓글 테이블과 작성자·접속표시 열을 추가합니다.
...
| --- | --- |
| `notices` | 공지사항 |
| `knowledge_posts` | 정보공유 글과 태그·링크·작성자 |
| `knowledge_comments` | 정보공유 글 댓글 |
| `project_rooms` | 프로젝트 방, 체크리스트, 자료, 회의 메모 |
| `project_updates` | 프로젝트별 진행사항과 작성자 |
| `project_comments` | 프로젝트별 댓글 |
...
- `INVITE_CODE`로 접속: 전체 페이지 조회, 정보공유 글·댓글 작성, 프로젝트 방 생성·수정, 프로젝트 진행사항·댓글·체크리스트·자료·회의 메모 작성
- `ADMIN_ACCESS_CODE`로 접속: 회원정보와 사이트 문구를 포함한 전체 관리, 게시물·댓글·프로젝트 삭제
- 잘못된 코드: `401` 응답 및 메인 페이지 접근 차단
- 역할 쿠키: `AUTH_SECRET`으로 서명한 HttpOnly 쿠키, 7일 유효

실제 접속코드는 README나 Git 추적 파일에 기록하지 말고 `.env.local`과 배포 환경변수에만 보관하세요. 무료 Vercel URL은 인터넷에서 접근 가능하므로 충분히 긴 임의 값을 사용하고 회원 외부에 공개하지 않는 편이 안전합니다. 회원정보 수정은 관리자에게만 허용됩니다.

게시물·댓글·프로젝트·진행사항에는 작성자가 직접 입력한 이름과 `접속표시 #XXXXXX`가 함께 표시됩니다. 접속표시는 서버가 요청 주소를 `AUTH_SECRET` 기반 HMAC으로 변환한 6자리 식별자이며, 원본 IP 주소는 데이터베이스에 저장하거나 화면에 표시하지 않습니다. 같은 네트워크를 공유하면 같은 표시가 나올 수 있으므로 로그인 계정이나 법적 신원 확인 수단으로 사용하면 안 됩니다.
...
1. 공용 회원 코드와 관리자 코드의 역할이 서로 다른지 확인합니다.
2. 관리자 창에서 테스트 공지를 작성합니다.
3. 다른 브라우저 또는 시크릿 창에 회원 코드로 접속해 새 공지가 보이는지 확인합니다.
4. 회원 창에서 회원정보 수정 버튼이 관리자에게만 보이는지 확인합니다.
5. 회원 창에서 정보공유 글·댓글, 프로젝트 생성·수정·진행사항·댓글을 저장하고 새로고침 후 유지되는지 확인합니다.
6. 브라우저 개발자 도구의 Application → Local Storage에 게시판·회원·프로젝트 데이터가 없는지 확인합니다.
7. 테스트 레코드를 삭제하고 두 브라우저 모두에서 삭제 결과를 확인합니다.
...
- `/knowledge`, `/knowledge/[id]` — 정보공유 검색·상세·작성자·댓글
- `/projects`, `/projects/[id]` — 프로젝트와 진행사항·댓글·체크리스트·자료·회의 메모
- `/gatherings` — 예정/지난 모임 및 일정 관리
- `/members` — 회원 활동 정보 조회 및 관리자 전용 편집
